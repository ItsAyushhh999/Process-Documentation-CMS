...
const reviewersIds = (props.data.collaborators)?.filter(user => user.flag == 1)?.map(item => item.collaborator_id);;
const assigneesIds = (props.data.collaborators)?.filter(user => user.flag == 0)?.map(item => item.collaborator_id);;
...
<template>
  ...
  <template v-else-if="isStatus">
    ...
    <AppChip v-else-if="data.value == '9'" color="purple">{{ data?.name }}</AppChip>
  ...
  <template v-else-if="isCollaborators">
    <div class="p-1" v-for="(elem, index) in data" :key="index">
