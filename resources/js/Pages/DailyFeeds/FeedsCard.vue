...
const taskStatus = computed(() => {
  return statusMapping[props.data.taskStatus];
});
...
<template>
  <div  class="flex flex-col space-y-6 mx-auto">
    <div  class="flex space-x-6 w-7/12 flex gap-8">
      <div class="flex flex-col items-center">
        ...
        >
          <img
            :src="`/storage/profiles/${data.profile_picture}`"
          ...
          <div class="z-20 w-full h-full overflow-hidden rounded-lg">
            <div class="flex justify-between items-center bg-gray-100 dark:bg-slate-800 py-2 px-3">
              <div class="flex gap-3">
                <span class="text-base dark:text-gray-300">
                  <a
                    :href="`tasks/${data.taskId}/edit`"
                    target="_blank"
                    v-if="data.taskId"
                    class="flex items-center gap-x-2 divide-x text-gray-500 dark:text-slate-400 hover:text-primary-500 dark:hover:text-primary-300"
                    ><span class=""
                      ><strong class="text-2xl">#{{ data.taskId }}</strong></span
                    ><span class="pl-2 line-clamp-1 text-2xl">{{
            ...
            <div class="px-3 py-2">
              <div class="flex gap-2 flex-wrap flex-1 mb-3">
                ...
                <app-chip :color="taskStatus.color">{{ taskStatus.value }}</app-chip>
              </div>
              <div
                class="p-4 rounded-lg border dark:border-slate-700 text-gray-500 whitespace-normal dark:text-gray-300 truncate mb-1"
              >
                <span v-html="data.taskDescription" class="description"></span>
              </div>
              <div v-for="(data, index) in data.activity" :key="index" class="mt-3">
              <div v-if="data.type === 'comment'" class="flex space-x-6  w-7/8 py-3 flex gap-8 ">
                    <div class="flex flex-col items-center ">
                        ...
                        <div
                            ...
                            <div class="z-20 w-full h-full overflow-hidden rounded-lg">
                                ...
                                <div class="px-3 py-2 ">
                                    <span >Comment :</span>
                                    <div
                class="p-4 rounded-lg border mt-2 dark:border-slate-700 text-gray-500 whitespace-normal dark:text-gray-300 truncate mb-1"
              >
                <span v-html="data.comments" class="description"></span>
              </div>
                        ...
                        <div v-show="data.replies" :class="{ hidden: data.replies.length === 0 }">
                            <div v-for="(reply, replyIndex) in data.replies" :key="replyIndex" >
                                ...
                                <div class="flex space-x-6 mb-3 ">
                                    ...
                                    <div
                                    class="border border-gray-300 dark:border-gray-600  grow relative rounded-lg  w-full bg-white dark:bg-slate-900">
                                        <div
                ...
                class="p-4 rounded-lg border mt-2 dark:border-slate-700 text-gray-500 whitespace-normal  dark:text-gray-300 truncate mb-1"
              >
                <span v-html="reply.comments" class="description"></span>
              </div>
                ...
                <div v-if="data.type === 'status'" class="flex  w-7/8 gap-8 space-x-6  ">
                    <div class="flex flex-col items-center ">
                        <div v-if="data.profile_picture"
                            class="w-10 h-10 rounded-full flex items-center justify-center text-white">
                            <img :src="`/storage/profiles/${data.profile_picture}`" title="{{data.profie_picture}}"
                ...
                <div v-if="data.type === 'activity'" class="flex w-7/8 gap-8 space-x-6 ">
      <div class="flex flex-col items-center">
        <div
          v-if="data.profile_picture"
          class="w-10 h-10 rounded-full flex items-center justify-center text-white"
      ...
      <div
        class="flex flex-col w-full  border border-gray-300 dark:border-gray-600 flex grow relative rounded-lg w-full bg-white dark:bg-slate-900"
      >
        <div class="z-20 w-full h-full overflow-hidden ">
            <div class="flex justify-between items-center bg-gray-100 dark:bg-slate-800 py-2 px-3">
                                    <div class="flex gap-3">
                                        <div
                                            class="inline-flex items-center gap-2 rounded-full  px-2 bg-blue-50 dark:bg-blue-600">
        ...
        <template v-if="data.property === '0'">
          <div class="px-3 py-2 flex items-center dark:text-slate-300">
            <div v-if="data.action === '0'">
              <span class="gap-1 m-1">Added </span><strong>{{ data.addedUser }}</strong
              ><span> as a </span><strong> assginee</strong>
            </div>
            <div v-if="data.action === '1'">
              <span class="gap-1 m-1">Removed </span><strong> {{ data.removedUser }}</strong
              ><span> from a </span><strong> assignee</strong>
            </div>
          </div>
        </template>
        <template v-else-if="data.property === '1'">
          <div class="px-3 py-2 flex items-center dark:text-slate-300">
            <div v-if="data.action === '0'">
              <span class="gap-1 m-1">Added </span><strong>{{ data.addedUser }}</strong
              ><span> as a </span><strong>reviewer</strong>
            </div>
            <div v-if="data.action === '1'">
              <span class="gap-1 m-1">Removed </span><strong>{{ data.removedUser }}</strong
              ><span> from a </span><strong>reviewer</strong>
            </div>
          </div>
        </template>
        ...
        <template v-else>
          <div class="px-3 py-2 flex items-center dark:text-slate-300">
            <div v-if="data.action === '0'">
              <span class="gap-1 m-1">Added </span><strong>{{ data.addedTaskTypes }}</strong
              ><span> as a </span><strong>taskTypes</strong>
            </div>
            <div v-if="data.action === '1'">
              <span class="gap-1 m-1">Removed </span><strong>{{ data.removedTaskTypes }}</strong
              ><span> from a </span><strong>taskTypes</strong>
            </div>
          </div>
        </template>
      </div>
</div>
<div class="relative w-7/8  flex gap-8  ">
    <div v-if="data.source">
      <div
        v-if="data.source === 'AWS'"
    ...
    <div
      v-if="data.source"
      class="border flex-1 rounded-lg bg-white dark:bg-slate-800 dark:border-slate-700 relative"
    >
      <div class="flex justify-between text-sm text-gray-500 px-5 pt-5">
        <a
          :href="`tasks/${data.task.id}/edit`"
          v-if="data.task"
          class="flex items-center gap-x-2 divide-x text-gray-500 dark:text-slate-400 hover:text-primary-500 dark:hover:text-primary-300"
          ><span class=""
            ><strong>#{{ data.task_id }}</strong></span
          ><span class="text-sm pl-2 line-clamp-1">{{ data.task ? data.task.title : '-' }}</span></a
        >
      ...
      <div class="px-5 pb-5 pt-2">
        <div class="flex flex-col gap-2 mb-6">
          ...
          <div v-if="data.project">
            <a
              :href="`project/${data.project.id}/document`"
              class="text-primary-500 dark:text-primary-300 hover:underline cursor-pointer transition-all text-sm"
              >{{ data.project.name }} <strong>({{ data.project.id }})</strong></a
            >
          </div>
        </div>
        <div class="flex items-center gap-2">
          <AppChip :color="getStatusColor(data.type)">{{ data.type }}</AppChip>
          <AppChip :color="getStatusColor(data.status)">{{
            convertStringToLowerCase(data.status)
          }}</AppChip>
        </div>
      </div>
      <div
