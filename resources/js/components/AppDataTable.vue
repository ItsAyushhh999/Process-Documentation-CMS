...
const emit = defineEmits(['onRowDrag'])
...
// const parentEl = useParentElement(table)
const { width, height } = useElementSize(parentContainer);
...
const dynamicTableHeight = () => {
  if (tableRefs.value) {
    const tableElement = tableRefs.value.getBoundingClientRect()
    tableHeight.value =
      window.innerHeight > tableElement.top - 320 ? window.innerHeight - tableElement.top : '640';

      console.log('tableHeight', tableHeight.value, tableElement.top, window.innerHeight)
  }
}
...
onMounted(() => {
  ...
  if (table.value) {
    tabulator.value = new Tabulator(table.value, {
      ...
      //   }
      // },
      rowHeader: props.movableRows
        ? {
            headerSort: false,
            resizable: false,
            // minWidth:30,
            width: 30,
            rowHandle: props.movableRows,
            formatter: 'handle'
          }
        : false,
      pagination: true,
    ...
    if(props.movableRows){
      tabulator.value.on("rowMoved", function(row){
        // console.log('row.getData()', row.getTable().getData())
        emit('onRowDrag', row.getTable().getData())
    });
    }
  }
});
...
//     );
//     // table.value.setData(filteredData)
//   }
// });
...
watch(searchQuery, () => {
  if (tabulator.value) {
    tabulator.value.setFilter(matchAny, { value: searchQuery.value.toLowerCase() })
...
onMounted(() => {
  ...
  if (tabulator.value) {
    tabulator.value.on('dataFiltered', function (filters, rows) {
      if (!isFiltering.value) {
        return;
      }
      if (!isUpdating && isFiltering.value) {
        isUpdating = true;
        if (rows.length > 0) {
          tabulator.value?.rowManager.refreshActiveData();
        }
        isUpdating = false;
      }
    });
...
<template>
  <div class="relative h-full" ref="parentContainer">
    <div class="flex flex-row items-end justify-between pb-2">
      <div class="w-96">
        <AppTextInput
          leading="mdi-light:home"
          type="text"
          v-model="searchQuery"
          placeholder="Search..."
          name="Search Datatable"
        />
      </div>
      <div class="flex items-center gap-x-4">
        <slot name="left"></slot>
        <app-button icon="fa-download" @click="downloadCSV" outline>CSV</app-button>
      </div>
    </div>
    <div ref="tableRefs" class="overflow-auto " :style="`height: ${tableHeight}px`">
      <div ref="table"></div>
    </div>
  </div>
</template>
<style>
.tabulator {
  @apply relative bg-white dark:bg-gray-800 border border-gray-300 overflow-hidden;
...
.tabulator .tabulator-header {
  @apply relative w-full bg-gray-100 outline-none whitespace-nowrap overflow-hidden font-bold dark:bg-gray-900 border-b border-gray-300;
}
...
@media (hover: hover) and (pointer: fine) {
  .tabulator
    ...
    @apply bg-gray-200 dark:bg-slate-800 cursor-pointer;
  }
}
...
.tabulator-row {
  box-sizing: border-box;
  min-height: 22px;
  @apply bg-white relative dark:bg-slate-900 border-b dark:border-slate-700;
}
...
.tabulator-row .tabulator-cell.tabulator-row-header {
  ...
  /* border-bottom: 1px solid #ddd; */
  /* background: #f9fafb; */
  @apply bg-gray-50 dark:bg-slate-800 dark:border-slate-700
}
...
.tabulator-header-filter input{
  @apply !border-gray-200 !rounded !font-medium !text-base !mt-2 !px-2 dark:bg-slate-800 dark:!border-slate-700;
}
.tabulator-cell a{
  @apply text-primary-500 font-semibold hover:underline dark:text-primary-300;
}
.tabulator-table{
